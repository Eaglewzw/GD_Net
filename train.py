...
def parse_args():
    ...
    parser.add_argument('--data-root', type=str,
                        default='/home/verser/Downloads/Drone_25k/MobileNet_Dataset',
                        help='数据集根目录')
    # 检查点保存目录
...
def main():
    ...
    # ------------------- 模型 -------------------
    model = YOLOv3_McuNet(cfg, device, num_classes=args.num_classes, trainable=True).to(device)
    ...
    # ------------------- 损失 / 优化器 / 调度器 -------------------
    criterion = build_criterion(cfg, device, num_classes=args.num_classes)
