...
class Criterion(object):
    ...
    def __call__(self, outputs, targets, epoch=0):
        ...
        # # 修正 flatten：防止重复乘 anchor 数
        # gt_objectness = gt_objectness[:, ::3, :].reshape(-1).to(device).float()
        # gt_classes = gt_classes[:, ::3, :].reshape(-1, self.num_classes).to(device).float()
        # gt_bboxes = gt_bboxes[:, ::3, :].reshape(-1, 4).to(device).float()
