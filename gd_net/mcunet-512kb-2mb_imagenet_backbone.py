...
def _make_backbone():
    json_file = osp.join(osp.split(osp.abspath(__file__))[0], "../mcunet_model/mcunet-512kb-2mb_imagenet.json")
    with open(json_file) as f:
        config = json.load(f)
    _model = ProxylessNASNets.build_from_config(config)
    ckpt = torch.load(osp.join(osp.split(osp.abspath(__file__))[0], "../mcunet_model/mcunet-512kb-2mb_imagenet.pth"))
    _model.load_state_dict(ckpt['state_dict'], strict=False)
    return _model


class mcunet_512kb_2mb(nn.Module):
...
if __name__ == "__main__":
    ...
    model = mcunet_512kb_2mb(0.33, 0.25)
    output = model(input)
    for k in output:
